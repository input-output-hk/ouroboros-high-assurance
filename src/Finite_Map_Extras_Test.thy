...
lemma inter_plus_dom:
  shows "fmdom' (m\<^sub>1 \<inter>\<^sub>+ m\<^sub>2) = fmdom' m\<^sub>1 \<inter> fmdom' m\<^sub>2"
  sorry
...
proof -
  ...
  also have "\<dots> =
    ...
    by (rule arg_cong, rule fmfilter_cong) (fastforce simp add: assms(2))
  ...
  also have "\<dots> = fmdom' m\<^sub>2 \<lhd>/ m\<^sub>1 ++\<^sub>f ({k $$:= v} ++\<^sub>f fmdom' m\<^sub>1 \<lhd>/ m\<^sub>2) ++\<^sub>f (m\<^sub>1 \<inter>\<^sub>+ m\<^sub>2)"
    by (simp only: fmadd_assoc)
  also have "\<dots> = fmdom' m\<^sub>2 \<lhd>/ m\<^sub>1 ++\<^sub>f fmdom' m\<^sub>1 \<lhd>/ m\<^sub>2 ++\<^sub>f {k $$:= v} ++\<^sub>f (m\<^sub>1 \<inter>\<^sub>+ m\<^sub>2)"
    by (simp add: assms(2) fmap_singleton_comm [symmetric])
  also have "\<dots> = fmdom' m\<^sub>2 \<lhd>/ m\<^sub>1 ++\<^sub>f fmdom' m\<^sub>1 \<lhd>/ m\<^sub>2 ++\<^sub>f ({k $$:= v} ++\<^sub>f (m\<^sub>1 \<inter>\<^sub>+ m\<^sub>2))"
    by (simp only: fmadd_assoc)
  also have "\<dots> = fmdom' m\<^sub>2 \<lhd>/ m\<^sub>1 ++\<^sub>f fmdom' m\<^sub>1 \<lhd>/ m\<^sub>2 ++\<^sub>f (m\<^sub>1 \<inter>\<^sub>+ m\<^sub>2) ++\<^sub>f {k $$:= v}"
    by (simp add: assms(2) fmap_singleton_comm [symmetric])
  finally show ?thesis .
qed
